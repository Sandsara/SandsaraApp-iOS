...
open class WOWTooltipView: UIView {
    ...
    var fillColor = UIColor(white: 1, alpha: 0.8) {
        didSet {
            setNeedsDisplay()
        }
    }
    
    var textColor = UIColor(white: 0.0, alpha: 0.8) {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    override open func draw(_ rect: CGRect) {
        ...
        let roundedRect = CGRect(x:bounds.origin.x, y:bounds.origin.y, width:bounds.size.width - 8, height:bounds.size.height * 0.8)
